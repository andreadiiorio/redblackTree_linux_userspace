...
/*
  ...
  linux/lib/rbtree.c
  userspace GNUC porting and minimaling dependencies:	Andrea Di Iorio
*/
		...
		if (node != sibling) {	/* node == parent->rb_left */
			if (rb_is_red(sibling)) {
				...
				__rb_rotate_set_parents(parent, sibling, root,
							RB_RED);
				///augment_rotate(parent, sibling); //TODO DEL_AUGMENTED_DEP
				sibling = tmp1;
			}
			...
			if (!tmp1 || rb_is_black(tmp1)) {
				...
				if (tmp1)
					rb_set_parent_color(tmp1, sibling,
							    RB_BLACK);
				///augment_rotate(sibling, tmp2); //TODO DEL_AUGMENTED_DEP
				tmp1 = sibling;
				sibling = tmp2;
			}
			/*
